...
class SplitDatasetLoader:
    """Load pre-extracted NSynth splits for training"""
    
    def __init__(self, splits_dir, sample_rate=16000):
        self.splits_dir = Path(splits_dir)
        self.sample_rate = sample_rate
        
        # Load global config
        config_path = self.splits_dir / 'config.json'
        ...
        print(f"Loaded dataset config from {splits_dir}")
    ...
    def load_split(self, split_name):
        ...
        for file_info in tqdm(file_list, desc=f"Loading {split_name}"):
            file_path = self.splits_dir / file_info['path']
        ...
        return np.array(audio_data, dtype=np.float32), np.array(labels, dtype=np.int32)
...
def main():
    parser = argparse.ArgumentParser(
        description='Train Worship Flow instrument classifier (Standalone)'
    )
